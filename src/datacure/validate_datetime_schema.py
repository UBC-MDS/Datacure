...
def validate_datetime_schema(df, columns, datetime_format, coerce_invalid=False):
    ...
    datetime_format : str
        Expected datetime format string used for strict validation
        (e.g., ``"%Y-%m-%d"``).
    ...
    dict
        ...
        status : {'pass', 'fail'}
            Overall validation status across all specified columns.
        validated_df : pandas.DataFrame
            A copy of the input DataFrame. If ``coerce_invalid=True``, the specified
            datetime columns are converted to pandas datetime dtype.
